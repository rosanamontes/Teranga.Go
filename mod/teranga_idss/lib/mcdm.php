...
{
	return elgg_echo("teranga_idss:karma:s{$term}");
}
...
function relativeUserExpertise($E_weight)
{
	...
	$base = elgg_get_plugin_setting('base_expertise', 'teranga_idss') / 100.0;
...
function userKarma($guid)
{
	$user = get_user($guid);
	if (!$user)
		return elgg_echo('teranga_idss:karma:none');
...
function userKarma_decisionMaking($valorationlist)
{
	...
	$enablePesos = elgg_get_plugin_setting('weight_assessments', 'teranga_idss');
	$C_weight = null;

	$enableExpertos = elgg_get_plugin_setting('weight_experts', 'teranga_idss');
	$E_weight = null;
