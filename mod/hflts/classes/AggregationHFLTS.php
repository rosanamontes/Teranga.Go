...
class AggregationHFLTS extends MCDM
{
    ...
    private function promedio()
    {
    	...
    	{
      		$index = key($values);
      		$this->ranking[$p]['promedio']['ref'] = $this->alternatives[$index] ;
      		$this->ranking[$p]['promedio']['tupla'] = toTuple( $candidato );
      		$this->ranking[$p]['promedio']['label'] = toLabel( $this->ranking[$p]['promedio']['tupla'][0] );
