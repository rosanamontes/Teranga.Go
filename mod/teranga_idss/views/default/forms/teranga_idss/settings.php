...
if (!elgg_is_xhr()) {
	echo '<p>' . elgg_echo('teranga_idss:settings:explanation') . '</p>';
}
...
foreach ($vars['data'] as $name => $info) 
{
	...
	if ($info['type'] == 'checkbox') 
	{
		...
		echo ' ' . elgg_echo("teranga_idss:label:$name", $echo_vars) . '</label>';
	} 
	else if ($info['type'] == 'range') 
	{
		echo $label . elgg_echo("teranga_idss:label:$name") . ' ';
		...
		echo '</label>';
		echo elgg_format_element('div', ['class' => 'teranga_idss-settings-range', 'id' => $name], $slider);
	}
	else
	{
		echo $label . elgg_echo("teranga_idss:label:$name") . ' ';
	...
	echo '<span class="elgg-text-help">' . elgg_echo("teranga_idss:help:$name") . '</span>';
	if ($info['readonly']) {
...
echo elgg_view('input/submit', array('value' => elgg_echo('teranga_idss:label:submit')));
echo '</div>';
...
<script type="text/javascript">
	...
	{
		$(".teranga_idss-settings-range").on('change', function(event)
		{
