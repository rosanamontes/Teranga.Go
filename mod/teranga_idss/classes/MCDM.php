...
abstract class MCDM 
{
	...
	public function getTitle() 
	{
		...
		$header = $this->label;
		$header = elgg_echo("teranga_idss:label:{$this->label}");
		return $header;
	}
	...
	public function getDescription() 
	{
		...
		$result = $this->label;
		$result = elgg_echo("teranga_idss:help:{$this->label}");
		return $result;
	}
	...
	*/
	public function run()
	{
		if (elgg_get_plugin_setting('debug', 'teranga_idss') == 1)
		...
		switch ($this->case)
		{
		 	case 'platform':
				...
				{
					//return ;
					register_error(elgg_echo("teranga_idss:mcdm:fail"));
					//forward(REFERER);
				}
	...
	*/
	...
	{ 	
		...
		//..-..-..-..-..-..-..-..-..-..-..-..-..-..-..-..-..-..-..-..-..-..-..-..-..-..- 
		if (elgg_get_plugin_setting('exportTex', 'teranga_idss') == 1)
	...
	*/
	...
	public function realEstateCase()
	{
		...
		$name = elgg_get_plugins_path() . "teranga_idss/samples/set_classic.csv";
		$this->parse_csv($name);		
	...
	public function todimCase()
	{
		...
		$name = elgg_get_plugins_path() . "teranga_idss/samples/set_todim.csv";
		$this->parse_csv($name);		
	...
	public function vikorCase($rangeTerms)
	{
		...
		if ($rangeTerms == "S7")
			$name = elgg_get_plugins_path() . "teranga_idss/samples/set_vikorS7.csv";
		else
			$name = elgg_get_plugins_path() . "teranga_idss/samples/set_vikor.csv";
		$this->parse_csv($name);		
	...
	public function electreCase()
	{
		...
		$name = elgg_get_plugins_path() . "teranga_idss/samples/set_electre.csv";
		$this->parse_csv($name);		
	...
	public function topsisCase($case)
	{
		...
		if ($case == "original")
		{
			...
			$name = elgg_get_plugins_path() . "teranga_idss/samples/set_topsis.csv";
		}
		else
		{
			$this->P=1; //num of experts
			...
			$name = elgg_get_plugins_path() . "teranga_idss/samples/set_topsis_benefit.csv";
		}
	...
	public function prometheeCase()
	{
		...
		$name = elgg_get_plugins_path() . "teranga_idss/samples/set_promethee.csv";
		$this->parse_csv($name);		
	...
	*/
	public function aggregate($assessments, $toHesitant)
	{
		...
		if ($aggOperator == 0)
		{
			//system_message(elgg_echo('teranga_idss:aggOperator:minmax'));//does not consider weights
			if ($toHesitant)
		...
		{
			//system_message(elgg_echo('teranga_idss:aggOperator:HLWA'));//admits weights 
