...
// create a new teranga_idss object
$entry = new ElggObject();
$entry->subtype = "teranga_idss";
$entry->title = $title;
...
$entry->tags = $tags;
// save to database and get id of the new agnaret_teranga_idss
$entry_guid = $entry->save();

// if the my_entry was saved, we want to display the new post
// otherwise, we want to register an error and forward back to the form
if ($entry_guid) {
	system_message(elgg_echo("teranga_idss:msgOk"));
	forward($entry->getURL());
} else {
	register_error(elgg_echo("teranga_idss:msgWrong"));
