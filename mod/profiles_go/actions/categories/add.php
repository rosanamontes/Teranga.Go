...
if (!empty($name) && preg_match("/^[a-zA-Z0-9_]{1,}$/", $name)) {
	...
	if (!empty($entity)) {
		...
		if ($entity->save()) {
			system_message(elgg_echo("profiles_go:action:category:add:succes"));
		} else {
			register_error(elgg_echo("profiles_go:action:category:add:error:save"));
		}
	} else {
		register_error(elgg_echo("profiles_go:action:category:add:error:object"));
	}
} else {
	register_error(elgg_echo("profiles_go:action:category:add:error:name"));
}
