...
if ($admin_option) {
	...
	echo "<div>" . elgg_view('input/checkboxes', array('name' => "notify", 'options' => array(elgg_echo('profiles_go:admin:adduser:notify') => 1))) . "</div>";
	echo "<div>" . elgg_view('input/checkboxes', array('name' => "use_default_access", 'value' => elgg_echo('profiles_go:admin:adduser:use_default_access'), 'options' => array(elgg_echo('profiles_go:admin:adduser:use_default_access') => 1))) . "</div>";
	...
	$categorized_fields = profiles_go_get_categorized_fields(null, true);
	$cats = $categorized_fields['categories'];
	$fields = $categorized_fields['fields'];
	
	if ($types || !empty($fields[0])) {
		echo "<a href='javascript:void(0);' onclick='$(\"#extra_metadata\").show(); $(this).hide();'>" . elgg_echo("profiles_go:admin:adduser:extra_metadata") . "</a>";
	...
	if ($types) {
		...
		$options = array();
		$options[""] = elgg_echo("profiles_go:profile:edit:custom_profile_type:default");
		...
		echo "<div>";
		echo "<label>" . elgg_echo("profiles_go:profile:edit:custom_profile_type:label") . "</label><br />";
