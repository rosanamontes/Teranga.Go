		...
		if ($configured_fields) {
			foreach ($configured_fields as $field) {
				...
				if (isset($_FILES[$metadata_name]) && $_FILES[$metadata_name]['error'] == 0) {
					...
					if ($filehandler->save()) {
						$filehandler->profiles_go_metadata_name = $metadata_name; // used to retrieve user file when deleting
					...
					if (empty($current_file_guid)) {
						...
						$options = array(
							...
							"limit" => 1,
							"metadata_name_value_pairs" => array("name" => "profiles_go_metadata_name", "value" => $metadata_name)
						);
