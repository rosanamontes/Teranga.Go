...
function trip_companions_sorter($a, $b) {
...
function get_suggestions($guid, $friends_of_friends_limit = 10, $groups_members_limit = 10) {
	...
	if ($friends_of_friends_limit) {
		...
		if (!is_array($fof)) system_error("Revisar foreach de lib/functions");
		foreach ($fof as $f) {
	...
	if ($groups_members_limit) {
		...
		if (!is_array($mog)) system_error("Revisar foreach de lib/functions2");
		foreach ($mog as $m) {
