...
function expages_init() {
	...
	elgg_register_page_handler('help', 'expages_page_handler');
...
function expages_public($hook, $handler, $return, $params){
	$pages = array('about', 'terms', 'privacy', 'help');
...
function expages_page_handler($page, $handler) {
	...
	//teranga traduccion
	if ($object[0]->description == "<p>teranga</p>")
	{
		//system_message("cargar traduccion de " . $object[0]->title);
		$content .= elgg_view('output/longtext', array('value' => elgg_echo("expages:content:{$object[0]->title}")));
	} 
	else if ($object) {
